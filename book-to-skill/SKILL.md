...
```
learn (编排层) ──调用──→ book-to-skill (原料层)
                              │
                      PDF/文档 → skill
                              │
                              ├──→ learn 继续调 knowledge-graph
                              └──→ knowledge-graph 也可能直接调 book-to-skill
```

本 skill 是 learn 学习流水线的**第一阶段**：将原始教材（PDF/EPUB/DOCX 等）转化为结构化的 knowledge skill，供 knowledge-graph 构建知识图谱时作为核对基准。

也可以被 `/knowledge-graph` 在阶段0 直接调用（当用户提供的是原始文档而非已有 skill 时）。

当然也可以完全独立使用：`/book-to-skill 教材.pdf`
...
> "📐 Technical mode selected — using opendataloader-pdf (AI-ready, fast, local) for structure-aware extraction with Docling as fallback (tables, code blocks, formulas preserved as markdown). Expect ~1s per page. Starting now…"

...
def extract_single_file(input_path: Path, extraction_mode: str, install_mode: str) -> dict:
    ...
    if ext == ".epub":
        ...
        if extraction_mode == "technical":
            print("Mode: technical — using opendataloader-pdf (AI-ready)...", end=" ", flush=True)
            text = extract_with_opendataloader_pdf(input_str)
            if text:
                method = "opendataloader-pdf"
                print("OK")
            else:
                print("not available")
                print("Mode: technical — trying pagewise OCR (pypdfium2 + rapidocr)...", end=" ", flush=True)
                text = extract_with_pagewise_ocr(input_str)
                if text:
                    method = "pagewise-ocr"
                    print("OK")
                else:
                    print("not available")
                    print("Mode: technical — trying Docling (layout-aware)...", end=" ", flush=True)
        ...
        if extraction_mode == "text" or not text:
            ...
            if text:
                ...
                print("not available")
                print("Trying opendataloader-pdf (AI-ready)...", end=" ", flush=True)
                text = extract_with_opendataloader_pdf(input_str)
                if text:
                    method = "opendataloader-pdf"
                    print("OK")
                    ...
                    print("not available")
                    print("Trying pagewise OCR (pypdfium2 + rapidocr)...", end=" ", flush=True)
                    text = extract_with_pagewise_ocr(input_str)
                    if text:
                        method = "pagewise-ocr"
                        print("OK")
                        ...
                        if text:
                            ...
                            if text:
                                ...
                                raise ExtractionError(
                                    ...
                                    "Install one of: poppler-utils (pdftotext), opendataloader-pdf, PyPDF2, or pdfminer.six\n"
